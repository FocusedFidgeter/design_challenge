...
class CreditCardPayment(PaymentStrategy):
    def pay(self, amount: Decimal) -> None:
        ...
        ccv_masked = len(ccv) * "*"
        print(
            f"Processing credit card payment of ${amount:.2f} with card number {card_number_masked} and expiration date {expiration_date} and CCV {ccv_masked}..."
        )
...
class PayPalPayment(PaymentStrategy):
    def pay(self, amount: Decimal) -> None:
        ...
        print(
            f"Processing PayPal payment of ${amount:.2f} with username {username} and password {password_masked}..."
        )
...
class ApplePayPayment(PaymentStrategy):
    def pay(self, amount: Decimal) -> None:
        ...
        print(
            f"Processing Apple Pay payment of ${amount:.2f} with device ID {device_id_masked}..."
        )
...
class ShoppingCart:
    ...
    def process_payment(self, payment_strategy: PaymentStrategy) -> None:
...
def main() -> None:
    ...
    if payment_type == "cc":
        ...
        payment_strategy = ApplePayPayment()
    else:
        print(f"Unknown payment type '{payment_type}'")
        return

    cart.process_payment(payment_strategy)
